...
docker build --no-cache -t hackaton-v2i-processing:v3 .

# Copiar imagem para o minikube
echo "📦 Copiando imagem para o minikube..."
minikube image load hackaton-v2i-processing:v3
...
kubectl rollout status deployment/v2i-processing-deployment-v1 --timeout=180s
